...
while(True):
    ...
    if(ret):
        ...
        if(handResult.multi_hand_landmarks):
            ...
            for i in range(21):
                handLandmark[i][0] = (1-handResult.multi_hand_landmarks[0].landmark[i].x) * imgWidth / imgSize
                handLandmark[i][1] = (1-handResult.multi_hand_landmarks[0].landmark[i].y) * imgHigh / imgSize
                handLandmark[i][2] = (handResult.multi_hand_landmarks[0].landmark[i].z) * imgWidth / imgSize
            
            handLandmarkDraw = numpy.zeros((3,21))
            
            for i in range(21):
                handLandmarkDraw[0][i] = handLandmark[i][0]
                handLandmarkDraw[1][i] = handLandmark[i][1]
                handLandmarkDraw[2][i] = handLandmark[i][2]

            ax.plot(handLandmarkDraw[0], handLandmarkDraw[2], handLandmarkDraw[1],'ro')

            for a, b in hancLandmarkConnection:
                ax.plot([handLandmark[a][0], handLandmark[b][0]], [handLandmark[a][2], handLandmark[b][2]], [handLandmark[a][1], handLandmark[b][1]])
