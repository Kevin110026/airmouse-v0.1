...
import random
import numpy as np
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') #use gpu if support
time_s=10
points_depth=3
points_hand=21
...
class Net(nn.Module): #CNN
    ...
    def forward(self, x): #using selu for testing, can be modified to see if its able to achieve better result
